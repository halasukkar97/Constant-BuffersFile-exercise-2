...
struct CONSTANT_BUFFER0
{
	...
	float scale;
	XMFLOAT2 packing_bytes;  //3x4 bytes = 12 bytes
};
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		else if (wParam == VK_UP)
		{
			...
			cb0_values.RedAmount -= 0.5f; //remove little bit of the redness
			cb0_values.scale += 0.1f;     // make it bigger
		...
		else if (wParam == VK_DOWN)
		{
			...
			cb0_values.RedAmount += 0.5f;  // add more redness
			cb0_values.scale -= 0.1f;      // make it smaller
